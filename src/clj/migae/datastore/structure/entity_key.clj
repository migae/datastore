...
(deftype PersistentEntityKeychain [content ^IPersistentMap em-meta]

  migae.datastore.IPersistentEntityKeychain
  ;; java.lang.Iterable
  (iterator [this]
  ...
  (^IPersistentMap meta [this]
    (do
      ;; (log/debug "PersistentEntityMap.meta" (get-keychain (.getKey content)))
      (let [md (into (some identity [em-meta {}]) {:migae/keychain (get-keychain (.getKey content))})]
        ;; (log/debug "meta: " md)
        md)))
  ;;;; extends IMeta
