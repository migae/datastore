(clojure.core/println "Start loading migae.datastore.types.entity_map_seq")
...
(deftype PersistentEntityMapSeq [content]
  ...
  (^ISeq ;;^clojure.lang.IPersistentVector
    cons  ;; -> ^ISeq ;;
    [this ^Object obj]
    (log/trace "PersistentEntityMapSeq.cons"))
  ...
  (nth [this i not-found]
    (log/trace "Indexed nth with not-found" i " nf: " not-found)
  ...
  (reduce [this ^IFn f ^Object to-map]  ; -> Object
    ...
    (log/trace "PersistentEntityMapSeq.reduce func:" f)
    (log/trace "PersistentEntityMapSeq.reduce to:" (class to-map) (type to-map))
    (cond
      (= (class to-map) clojure.lang.PersistentArrayMap$TransientArrayMap)
      (do
        ;; apply f to this and first elt of to-map, etc
;;        (log/trace "PersistentEntityMapSeq.reduce to-map first: " (first (persistent! to-map)))
        (map
         #(log/trace "PersistentEntityMapSeq.reduce to-map item: " %)
         to-map)
        to-map)
